#!/bin/sh
# Instruction counts for the Kairos kernel, under callgrind. Run inside WSL.
#
...
bin=target/release/kernel-ir
cargo build --release
[ -f "$bin" ] || { echo "no binary at $bin" >&2; exit 1; }

# Every source that can change the count must be OLDER than the binary.
newest=$(find ../../crates/rusty_rtos_demo-core/src src -name '*.rs' -newer "$bin" -print -quit)
if [ -n "$newest" ]; then
...
valgrind --tool=callgrind --callgrind-out-file=callgrind.out \
         --cache-sim=no --branch-sim=no "./$bin" 2>&1 | grep -E "checksum|scenarios|refs:"
echo "--- per file ---"
