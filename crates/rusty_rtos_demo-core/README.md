...
- **The corpus**: **22 scenarios** identical to the C kernel's own trace at
  the pinned length (`kairos conform --all`), of which **18 run byte-identically
  on four architectures** — host, ARMv7-M, RV32 and Xtensa LX7, the last on
  silicon. The host runs all 18 for **3,600,000 ticks**, an hour at the
  oracle's own tick rate.
